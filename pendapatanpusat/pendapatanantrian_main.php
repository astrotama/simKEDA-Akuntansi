...
function pendapatanantrian_main($arg=NULL, $nama=NULL) {
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			$kodeuk = $_SESSION["pendapatanantrian_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		$bulan = $_SESSION["pendapatanantrian_bulan"];
		if ($bulan=='') $bulan = "date('n')";
		
		$hari = $_SESSION["pendapatanantrian_hari"];
		if ($hari=='') $hari = '0';
		
		$statusjurnal = $_SESSION["pendapatanantrian_statusjurnal"];
		if ($statusjurnal=='') $statusjurnal = '0';
		
		$keyword = $_SESSION["pendapatanantrian_keyword"];
		if ($keyword == '') $keyword = ' ';
		/*
...
function pendapatanantrian_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["pendapatanantrian_kodeuk"] = $kodeuk;
	$_SESSION["pendapatanantrian_bulan"] = $bulan;
	$_SESSION["pendapatanantrian_hari"] = $hari;
	$_SESSION["pendapatanantrian_statusjurnal"] = $statusjurnal;
	$_SESSION["pendapatanantrian_keyword"] = $keyword;
