<?php
function pendapatanmanuk_post_main($arg=NULL, $nama=NULL) {
	
	$output_form = drupal_get_form('pendapatanmanuk_post_main_form');
...
function pendapatanmanuk_post_main_form($form, &$form_state) {
...
function pendapatanmanuk_post_main_form_submit($form, &$form_state) {
	if ($form_state['clicked_button']['#value'] == $form_state['values']['detiladd']) {
...
function _load_rekening() {
	...
	$query = db_select("rincianobyek", "r");
	$query->innerJoin("anggperda", "a", "r.kodero=a.kodero");
	$query->fields("r", array('kodero', 'uraian'));
	$query->condition("a.kodero", db_like("61") . "%", "LIKE");
	$query->orderBy("r.kodero");
