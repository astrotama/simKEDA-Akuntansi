<?php
function laporan_detilpro_main($arg=NULL, $nama=NULL) {
    $cetakpdf = '';
	...
	if ($jenis == 'pdf') {		
		$output = gen_report_realisasi_print($bulan, $kodeuk, $kodepro);
		...
		$results = db_query('select program from {program} where kodepro=:kodepro', array(':kodepro' => $kodepro));
		...
		$output = gen_report_realisasi($bulan, $kodeuk, $kodepro);

		$btn = l('Cetak', 'laporandetilpro/filter/' . $bulan . '/' . urlencode($kodeuk) . '/' . $kodepro . '/' . $margin . '/' . $marginkiri . '/pdf' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-primary')));
...
function gen_report_realisasi($bulan, $kodeuk, $kodepro) {
...
$query->fields('k', array('kodepro', 'kodekeg', 'kegiatan'));
$query->addExpression('SUM(ag.jumlah)', 'anggaran');
$query->condition('k.kodepro', $kodepro, '='); 
$query->condition('k.kodeuk', $kodeuk, '='); 
$query->groupBy('k.kodekeg');
...
foreach ($results_keg as $data_keg) {
	...
	$kodepro_view = $data_keg->kodepro . '.' . substr($data_keg->kodekeg,-3);
	
	$rows[] = array(
		array('data' => '<strong>' . $kodepro_view . '</strong>', 'align' => 'left', 'valign'=>'top'),
	...
	foreach ($results_rek as $data_rek) {
		...
		$rows[] = array(
			array('data' => $kodepro_view . '.' . $data_rek->kodero, 'align' => 'left', 'valign'=>'top'),
...
function gen_report_realisasi_print($bulan, $kodeuk, $kodepro) {
...
$rows[] = array(
	array('data' => $program, 'width' => '510px', 'colspan'=>6, 'align'=>'center','style'=>'font-size:110%;border:none'),
);
$rows[] = array(
	array('data' => $namauk, 'width' => '510px', 'colspan'=>6, 'align'=>'center','style'=>'font-size:80%;border:none'),
);
...
foreach ($results_keg as $data_keg) {
	...
	$kodepro_view = $data_keg->kodepro . '.' . substr($data_keg->kodekeg,-3);
	$rows[] = array(
		array('data' => $kodepro_view, 'width' => '70px', 'align'=>'left','style'=>'font-size:80%;border-left:1px solid black;border-right:1px solid black;'),
	...
	foreach ($results_rek as $data_rek) {
			...
			$rows[] = array(
				array('data' => $kodepro_view . '.' . $data_rek->kodero, 'width' => '70px', 'align'=>'left','style'=>'font-size:80%;border-left:1px solid black;border-right:1px solid black;'),
