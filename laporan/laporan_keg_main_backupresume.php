...
$query->addExpression('SUM(ag.jumlah)', 'anggaran'); $query->addExpression('SUM(ag.jumlahsesudah)', 'realisasi');
...
foreach ($results as $datas) {
	...
	foreach ($results_kel as $data_kel) {
		...
		$query->addExpression('SUM(keg.totalsesudah)', 'realisasi');
		$query->condition('keg.kodeuk', $kodeuk, '='); 
		$query->condition('keg.jenis', '2', '='); 
		$query->condition('keg.inaktif', '0', '=');
		...
		foreach ($results_pro as $data_pro) {
			...
			$query->fields('keg', array('kodekeg', 'kegiatan', 'total', 'totalsesudah'));
			$query->condition('keg.kodeuk', $kodeuk, '='); 
			...
			foreach ($results_keg as $data_keg) {
				...
				$realisasi = $data_keg->totalsesudah;
