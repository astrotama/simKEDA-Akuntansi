		...
		$btn = '<div class="btn-group">' .
				...
				'</button>' .
					'<ul class="dropdown-menu">' .
						'<li><a href="/laporandetil/filter/' . $bulan . '/' . urlencode($kodeuk) . '/' . $akun . '/' . $margin . '/' . $marginkiri . '/kum">Realisasi Kumulatif</a></li>' .
						'<li><a href="/laporandetil/filter/' . $bulan . '/' . urlencode($kodeuk) . '/' . $akun . '/' . $margin . '/' . $marginkiri . '/per">Realisasi Periodik</a></li>' .
					'</ul>' .
				'</div>';		
		$btn .= '&nbsp;<div class="btn-group">' .
				...
				'</button>' .
					'<ul class="dropdown-menu">' .
						'<li><a href="/laporandetil/filter/' . $bulan . '/' . urlencode($kodeuk) . '/' . $akun . '/' . $margin . '/' . $marginkiri . '/excelkum">Realisasi Kumulatif</a></li>' .
						'<li><a href="/laporandetil/filter/' . $bulan . '/' . urlencode($kodeuk) . '/' . $akun . '/' . $margin . '/' . $marginkiri . '/excelper">Realisasi Periodik</a></li>' .
					'</ul>' .
				'</div>';		
...
if (substr($kodej, 0,1)=='4') {
	...
	foreach ($results_oby as $data_oby) {
		...
		$uraian = l($data_oby->uraian, '/akuntansi/buku/ZZ/'  . $data_oby->kodeo  . '/'  . $kodeuk . '/' . $tanggal_awal . '/' . $tanggal_akhir . '/', array('attributes' => array('class' => null)));
...
$rows[] = array(
	array('data' => $rekening, 'width' => '510px', 'colspan'=>6, 'align'=>'center','style'=>'font-size:110%;border:none'),
);
$rows[] = array(
	...
	foreach ($results_oby as $data_oby) {
		...
		foreach ($results_rek as $data_rek) {
			...
			$rows[] = array(
				...
				array('data' => apbd_fn($realisasi), 'width' => '70px', 'align'=>'right','style'=>'font-size:80%;border-right:1px solid black;'),
				array('data' => apbd_fn1(apbd_hitungpersen($data_rek->anggaran, $realisasi)), 'width' => '30px', 'align'=>'right','style'=>'font-size:80%;border-right:1px solid black;'),
				array('data' => apbd_fn1($data_rek->anggaran - $realisasi), 'width' => '70px', 'align'=>'right','style'=>'font-size:80%;border-right:1px solid black;'),
			);
...
$rows[] = array(
	array('data' => $rekening, 'width' => '510px', 'align'=>'center','style'=>'font-size:110%;border:none'),
);
$rows[] = array(
...
$results = db_query('select uraian from {obyek} where kodeo=:kodeo', array(':kodeo' => $kodeo));
foreach ($results as $datas) {
