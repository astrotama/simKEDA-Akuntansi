...
function laporan_fu_main($arg=NULL, $nama=NULL) {
	...
	if ($cetakpdf=='pdf_uspk') {
		...
		$output = gen_report_realisasi_print('9', $tanggal, '2');
		...
		//$output = gen_report_realisasi_print_urusan_program_kegiatan($tanggal,  $ttdlaporan, $index);
		//$output = gen_report_realisasi_print_fungsi_urusan_skpd($tanggal,  $ttdlaporan, $index);
		$output = gen_report_realisasi_urusan('9');
		$output_form = drupal_get_form('laporan_fu_main_form');	
...
foreach ($results as $datas) {
	...
	$sql->addExpression('SUM(k.agg_511+k.agg_513+k.agg_514+k.agg_515+k.agg_516+k.agg_517+k.agg_518)', 'agg_btl');
	$sql->addExpression('SUM(k.rea_511+k.rea_513+k.rea_514+k.rea_515+k.rea_516+k.rea_517+k.rea_518)', 'rea_btl');

	$sql->addExpression('SUM(k.agg_521+k.agg_522+k.agg_523)', 'agg_bl');
	$sql->addExpression('SUM(k.rea_521+k.agg_522+k.rea_523)', 'rea_bl');
	
	$sql->condition('u.kodef', $datas->kodef, '='); 
	
	$res = $sql->execute();
	foreach ($res as $data) {
		$btl_agg = $data->agg_btl;
		$bl_agg = $data->agg_bl;

		$btl_rea = $data->rea_btl;
		$bl_rea = $data->rea_bl;
	}
