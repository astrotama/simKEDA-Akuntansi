...
function laporan_detiluk_main($arg=NULL, $nama=NULL) {
	...
	if ($cetakpdf=='pdf') {
		if (substr($akun,0,1)=='4') {
			$output = gen_report_realisasi_pendapatan_print($akun, $bulan);
			apbd_ExportPDF_P($output, 10, "LAP.pdf");
		} else {
			$output = gen_report_realisasi_belanja_print($akun, $bulan);
			apbd_ExportPDF_P($output, 10, "LAP.pdf");
		}
		...
		if (substr($akun,0,1)=='4') {
			$output = gen_report_realisasi_pendapatan($akun, $bulan);
		} else {
			$output = gen_report_realisasi_belanja($akun, $bulan);
		}
		echo $output;
...
function laporan_detiluk_main_form_submit($form, &$form_state) {
...
function laporan_detiluk_main_form($form, &$form_state) {
...
function gen_report_realisasi_pendapatan($akun, $bulan) {
...
foreach ($results as $datas) {
	...
	$namasingkat = l($datas->namasingkat, '/akuntansi/buku/ZZ/'  . $akun  . '/'  . $datas->kodeuk . '/' . $tanggal_awal . '/' . $tanggal_akhir . '/', array('attributes' => array('class' => null)));
	$rows[] = array(
...
function gen_report_realisasi_pendapatan_print($akun, $bulan) {
...
function gen_report_realisasi_belanja($akun, $bulan) {
...
function gen_report_realisasi_belanja_print($akun, $bulan) {
