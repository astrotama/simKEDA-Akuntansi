<?php
function laporan_rekonkas_main($arg=NULL, $nama=NULL) {
	...
	if ($filter=='filter') {
		...
		$output_form = drupal_get_form('laporan_rekonkas_main_form');	
...
function laporan_rekonkas_main_form_submit($form, &$form_state) {
	$bulan= $form_state['values']['bulan'];
	$kodeuk = $form_state['values']['kodeuk'];
	$tingkat = $form_state['values']['tingkat'];

	if($form_state['clicked_button']['#value'] == $form_state['values']['submit']) {
		$uri = 'laporanrekonkas/filter/' . $bulan . '/'. $kodeuk . '/' . $tingkat;
	} else if($form_state['clicked_button']['#value'] == $form_state['values']['submitkeg']) {
		$uri = 'laporanrekonkas/filterkeg/' . $bulan . '/'. $kodeuk . '/' . $tingkat;
	} else {
		$uri = 'laporanrekonkas/filteruk/' . $bulan . '/'. $kodeuk . '/' . $tingkat;
	}	
	drupal_goto($uri);
...
function laporan_rekonkas_main_form($form, &$form_state) {
...
$header = array (
	...
	array('data' => 'SPJ', 'width' => '90px', 'valign'=>'top'),
...
foreach ($results as $datas) {
	...
	$sql = db_select('bendahara', 'j');
	$sql->innerJoin('bendaharaitem', 'ji', 'j.bendid=ji.bendid');
...
$header = array (
	...
	array('data' => 'SPJ', 'width' => '80px', 'valign'=>'top'),
...
foreach ($results as $datas) {
	...
	//laporanrekonkas/filter/12/57/3

	$skpd = l($datas->namauk, '/laporanrekonkas/filter/'  . $bulan  . '/'  . $datas->kodeuk . '/3', array('attributes' => array('class' => null)));
