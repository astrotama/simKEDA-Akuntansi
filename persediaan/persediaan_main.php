<?php
function persediaan_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			$kodeuk = $_SESSION["persediaann_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		$bulan = $_SESSION["persediaan_bulan"];
		if ($bulan=='') $bulan = '1';
		
		$keyword = $_SESSION["keyword_persediaan"];
	...
	$output_form = drupal_get_form('persediaan_main_form');
	$header = array (
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_jurnal('persediaan/jurnal/' . $data->jurnalid);
	...
	//BUTTON
	//$btn = apbd_button_baru('/persediaan/jurnal');
	$btn = l('<span class="glyphicon glyphicon-file" aria-hidden="true"></span> Penerimaan', '/persediaan/jurnal/0' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-primary btn-sm')));
	$btn .= "&nbsp;" . l('<span class="glyphicon glyphicon-file" aria-hidden="true"></span> Pemakaian', '/persediaan/jurnal/1' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-warning btn-sm')));
	$btn .= "&nbsp;" . l('<span class="glyphicon glyphicon-file" aria-hidden="true"></span> Pengeluaran', '/persediaan/jurnal/2' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-success btn-sm')));
...
function persediaan_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["persediaann_kodeuk"] = $kodeuk;
	$_SESSION["persediaan_bulan"] = $bulan;
	$_SESSION["keyword_persediaan"] = $keyword;
	
	$uri = 'persediaan/filter/' . $kodeuk . '/' . $bulan . '/' . $keyword;
	drupal_goto($uri);
	
}


function persediaan_main_form($form, &$form_state) {
	/*
	$kodeuk = 'ZZ';
	//$bulan = date('m');
	$bulan = '11';
