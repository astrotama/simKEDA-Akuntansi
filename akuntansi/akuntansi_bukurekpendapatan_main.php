<?php
function akuntansi_bukurekpendapatan_main($arg=NULL, $nama=NULL) {
   

	$output_form = drupal_get_form('akuntansi_bukurekpendapatan_main_form');
	return drupal_render($output_form);
	
}

function akuntansi_bukurekpendapatan_main_form ($form, &$form_state) {
	$kodej = arg(1);	
	$kodeo = arg(2);
	$kodero = arg(2);
...
function _load_jenis() {
	...
	$or->condition("j.kodek", db_like('4') . '%', 'LIKE');
	if (isSuperuser()) $or->condition("j.kodej", db_like('61') . '%', 'LIKE');
	...
	$query->condition("j.kodek", db_like('61') . '%', 'LIKE');
...
function akuntansi_bukurekpendapatan_main_form_validate($form, &$form_state) {
...
function akuntansi_bukurekpendapatan_main_form_submit($form, &$form_state) {
