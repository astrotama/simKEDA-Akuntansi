...
function getDataView($kodekeg, $kodero, $kodeuk, $tglawal, $tglakhir) {
	...
	if  ((substr($kodero,0,1)=='4') or (substr($kodero,0,1)=='5') or (substr($kodero,0,1)=='7'))
		$suffix_item = 'lra';
	else
		$suffix_item = 'lo';
	...
	//SEBELUMNYA
	$query = db_select('jurnalitem' . $suffix_item, 'ji');
	$query->innerJoin('jurnal' . $suffix_jurnal, 'j', 'ji.jurnalid=j.jurnalid');
	...
	$query = db_select('jurnalitem' . $suffix_item, 'ji')->extend('TableSort');
	$query->innerJoin('jurnal' . $suffix_jurnal, 'j', 'ji.jurnalid=j.jurnalid');
...
function getDataPrint($kodekeg, $kodero, $kodeuk, $tglawal, $tglakhir) {
	...
	$top[] = array(
			array('data' => 'BUKU BESAR SAP','width' => '510px', 'align'=>'center','style'=>'border:none;'),
			);
