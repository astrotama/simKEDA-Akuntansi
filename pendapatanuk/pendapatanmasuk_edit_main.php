<?php
function pendapatanmasuk_edit_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){			  
		...
		$btn = l('Cetak', 'pendapatanjurnaluk/edit/' . $jurnalid . '/pdf' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-primary')));
		//$btn .= "&nbsp;" . l('Excel', '' , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-primary')));
		
		//$output = theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('pendapatanmasuk_edit_main_form');
...
function pendapatanmasuk_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'edit') !== false) {
		$referer = $_SESSION["pendapatanjurnaluklastpage"];
	} else {
		$_SESSION["pendapatanjurnaluklastpage"] = $referer;
	}
...
function pendapatanmasuk_edit_main_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['submithapus']) {
		...
		try {
			...
			//JURNAL ITEM LRA
			//1
			$query = db_update('jurnalitemlrauk')
					->fields(
						array(
							'debet' => $jumlah,
						)
			...
			//JURNAL ITEM LO
			//1
			$query = db_update('jurnalitemlouk')
					->fields(
						array(
							'debet' => $jumlah,
						)
