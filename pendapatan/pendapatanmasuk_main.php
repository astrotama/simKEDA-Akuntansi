...
function pendapatanmasuk_main($arg=NULL, $nama=NULL) {
	...
	$query->fields('a', array('setorid', 'kodeuk', 'tanggal', 'jurnalsudah', 'keterangan', 'jumlahmasuk', 'jurnalid'));
	...
	foreach ($results as $data) {
		...
		if($data->jurnalsudah=='1'){
			...
			$editlink = apbd_button_jurnal('pendapatanmasuk/jurnaledit/' . $data->jurnalid);
...
function pendapatanmasuk_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$uri = 'pendapatanmasuk/filter/' . $kodeuk . '/' . $bulan . '/' . $statusjurnal . '/' . $keyword;
	drupal_goto($uri);
