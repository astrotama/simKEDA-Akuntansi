<?php
function belanjasumberdana_main($arg=NULL, $nama=NULL) {
	...
	$output_form = drupal_get_form('belanjasumberdana_main_form');
	$header = array (
	...
	$query->orderBy('k.sumberdana', 'ASC');
	$query->limit($limit);	
	...
	foreach ($results as $data) {
		...
		$keterangan = l($data->jumlahkegiatan . ' Kegiatan <span class="glyphicon glyphicon-th-large" 
						aria-hidden="true"></span>' , 'belanja/filter/' . $bulan . '/ZZ/' . $kodek . '/' . $data->sumberdana , array ('html' => true, 'attributes'=> array ('class'=>'text-success pull-right')));
		
		$rows[] = array(
						array('data' => $no, 'width' => '10px', 'align' => 'right', 'valign'=>'top'),
						array('data' => $data->sumberdana . $keterangan, 'align' => 'left', 'valign'=>'top'),
	...
	//$btn .= apbd_button_chart('belanja/chart/' . $bulan.'/ZZ/SEMUA/SEMUA/sumberdana_tot/single');
...
function belanjasumberdana_main_form_submit($form, &$form_state) {
	$bulan= $form_state['values']['bulan'];
	$kodek = $form_state['values']['kodek'];
	
	$uri = 'belanjasumberdana/filter/' . $bulan.'/' . $kodek;
	drupal_goto($uri);
	
}


function belanjasumberdana_main_form($form, &$form_state) {
