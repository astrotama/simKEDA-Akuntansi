<?php
function belanjarek_keg_main($arg=NULL, $nama=NULL) {
	...
	$query->fields('k', array('kodekeg','kegiatan', 'kodeskpd', 'namaskpd'));
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_bukubesar('akuntansi/buku/'. $bulan .'/' . $data->kodekeg . '/'.$kodero . '/' . $data->kodeskpd);
	...
	$output_form = drupal_get_form('belanjarek_keg_main_form');
...
function belanjarek_keg_main_form($form, &$form_state) {
	...
	if ($kodeuk!='ZZ') $query->condition('kodeskpd',$kodeuk,'=');
