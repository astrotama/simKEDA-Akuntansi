<?php
function belanjafungsi_main($arg=NULL, $nama=NULL) {
	...
	$output_form = drupal_get_form('belanjafungsi_main_form');
	$header = array (
		array('data' => 'No','width' => '10px', 'valign'=>'top'),
		array('data' => 'Kode', 'field'=> 'kodefungsi',  'width' => '10px',  'valign'=>'top'), 
		array('data' => 'Fungsi',  'field'=> 'namafungsi', 'valign'=>'top'), 
	...
	$query->fields('k', array('kodefungsi','namafungsi'));
	...
	$query->groupBy('kodefungsi');
	$query->groupBy('namafungsi');

	$query->orderByHeader($header);
	$query->orderBy('k.kodefungsi', 'ASC');
	$query->limit($limit);	
	...
	foreach ($results as $data) {
		...
		//$keterangan = l($data->jumlahkegiatan . ' Kegiatan <span class="glyphicon glyphicon-th-large" aria-hidden="true"></span>' , 'belanja/filter/' . $bulan . '/' . $data->kodeskpd . '/' . $kodek . '/SEMUA' , array ('html' => true, 'attributes'=> array ('class'=>'text-success pull-right')));
		//$keterangan = l($data->jumlahkegiatan . ' Kegiatan <span class="glyphicon glyphicon-th-large" 
		//				aria-hidden="true"></span>' , 'belanja/filter/' . $bulan . '/ZZ/' . $kodek . '/SEMUA/F' . $data->kodefungsi , array ('html' => true, 'attributes'=> array ('class'=>'text-success pull-right')));

		$keterangan = l($data->jumlahkegiatan . ' Kegiatan <span class="glyphicon glyphicon-th-large" 
						aria-hidden="true"></span>' , 'belanja/filter/' . $bulan . '/ZZ/' . $kodek . '/SEMUA/F' . $data->kodefungsi , 
						array ('html' => true, 'attributes'=> array ('class'=>'text-success pull-right')));

		//$keterangan .= l($data->jumlahskpd . ' SKPD&nbsp;', 
		//				'belanjauk/filter/' . $bulan . '/SEMUA/F' . $data->kodefungsi , 
		//				array ('html' => true, 'attributes'=> array ('class'=>'text-info pull-right')));
		
		$rows[] = array(
						array('data' => $no, 'width' => '10px', 'align' => 'right', 'valign'=>'top'),
						array('data' => $data->kodefungsi, 'align' => 'left', 'valign'=>'top'),
						array('data' => $data->namafungsi . $keterangan, 'align' => 'left', 'valign'=>'top'),
	...
	//$btn .= apbd_button_chart('belanja/chart/' . $bulan.'/ZZ/SEMUA/SEMUA/fungsi_tot/single');
...
function belanjafungsi_main_form_submit($form, &$form_state) {
	$bulan= $form_state['values']['bulan'];
	$kodek = $form_state['values']['kodek'];
	
	$uri = 'belanjafungsi/filter/' . $bulan.'/' . $kodek;
	drupal_goto($uri);
	
}


function belanjafungsi_main_form($form, &$form_state) {
