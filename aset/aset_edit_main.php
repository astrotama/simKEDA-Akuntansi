<?php
function aset_edit_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){			  
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('aset_edit_main_form');
...
function aset_edit_main_form($form, &$form_state) {
	...
	if ($tipe=='') {
		$tipe = '0';
		$title = 'Jurnal Penerimaan Barang';	
	} else if ($tipe=='1') 
		$title = 'Jurnal Penyusutan';	
	else
		$title = 'Jurnal Pengeluaran Barang';		
	...
	$form['formapbd'] = array (
		...
		$query->fields('r', array('kodero','uraian'));
		$query->condition('r.kodero', db_like('132') . '%', 'LIKE');
		$results = $query->execute();
		$i = 0;
		foreach ($results as $data) {
...
function aset_edit_main_form_submit($form, &$form_state) {
